...
if [[ "$2" == "gapbs-bc" ]]; then
        ...
        BENCH="${BENCH_PATH}/bc -f ${BENCH_PATH}/pregen_g28.sg -n 4"
	...
	echo 1G > ${CGMEM_DIR}/memory.max_at_node0
elif [[ "$2" == "xindex" ]]; then
	...
	echo 10G > ${CGMEM_DIR}/memory.max_at_node0
        ...
        BENCH="${BENCH_PATH}/out-perf.masstree/benchmarks/dbtest --verbose --bench ycsb --num-threads 20 --scale-factor 200000 --ops-per-worker=100000000 --slow-exit"
	echo 10G > ${CGMEM_DIR}/memory.max_at_node0
	...
	echo 4587M > ${CGMEM_DIR}/memory.max_at_node0
