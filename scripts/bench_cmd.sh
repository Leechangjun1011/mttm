...
if [[ "$1" == "gapbs-bc" ]]; then
        if [[ "$2" == "mix1" ]]; then
                ...
                BENCH="${GAPBS_DIR}/bc -f ${GAPBS_DIR}/pregen_g28.sg -n 8"
        fi
        ...
        if [[ "$2" == "mix1" || "$2" == "mix4" ]]; then
                ...
                BENCH="${GAPBS_DIR}/pr -f ${GAPBS_DIR}/pregen_g28.sg -i 1000 -t 1e-4 -n 8"
        fi
elif [[ "$1" == "xsbench" ]]; then
        if [[ "$2" == "mix1" ]]; then
                BENCH="${XSBENCH_DIR}/XSBench -t 8 -g 70000 -p 30000000"
        else
                BENCH="${XSBENCH_DIR}/XSBench -t 8 -g 70000 -p 25000000"
        fi
elif [[ "$1" == "xindex" ]]; then
        if [[ "$2" == "mix2" ]]; then
                ...
                BENCH="${XINDEX_DIR}/build/ycsb_bench --fg 6 --iteration 20"
        fi
elif [[ "$1" == "btree" ]]; then
        ...
        if [[ "$2" == "mix4" ]]; then
                ...
                BENCH="${SILO_DIR}/out-perf.masstree/benchmarks/dbtest --verbose --bench ycsb --num-threads 8 --scale-factor 240000 --ops-per-worker=450000000"
        fi
elif [[ "$1" == "cpu_dlrm_small_low" ]]; then
        if [[ "$2" == "mix3" ]]; then
                BENCH="bash ${CPU_DLRM_DIR}/dp_ht_8c.sh small low mix3"
        else
                BENCH="bash ${CPU_DLRM_DIR}/dp_ht_8c.sh small low"
        fi
elif [[ "$1" == "cpu_dlrm_small_high" ]]; then
        if [[ "$2" == "mix2" ]]; then
                BENCH="bash ${CPU_DLRM_DIR}/dp_ht_8c.sh small high mix2"
        else
                BENCH="bash ${CPU_DLRM_DIR}/dp_ht_8c.sh small high"
        fi
elif [[ "$1" == "fotonik" ]]; then
